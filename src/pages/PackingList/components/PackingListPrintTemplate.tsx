...
export const PackingListPrintTemplate: React.FC<Props> = ({ data, isPreview = false }) => {
    return (
        <div className={`print-template ${isPreview ? 'block relative p-4 sm:p-10 print:hidden shadow-xl' : 'hidden print:block print:absolute print:inset-0 z-[100000] p-10 print:p-0 print:m-0'} bg-white text-black w-full print:h-auto overflow-visible font-serif text-[13px] max-w-[210mm] mx-auto`} style={{ minHeight: '297mm' }}>
            <div className="max-w-[190mm] mx-auto mt-8 flex flex-col pt-8">
                ...
                {/* Items Table */}
                <div className="border-t border-b border-black py-2 flex text-center font-bold uppercase">
                    <div className="w-[150px] text-left">SHIPPING MARKS</div>
                    <div className="flex-1 text-left pl-4">QUANTITY & DESCRIPTIONS</div>
                    <div className="w-[120px]">NET WEIGHT</div>
                    <div className="w-[120px]">GROSS WEIGHT</div>
                </div>

                <div className="flex pt-4 min-h-[300px]">
                    <div className="w-[150px] whitespace-pre-wrap text-[13px]">{data?.shippingMarks}</div>
                    <div className="flex-1 pl-4 whitespace-pre-wrap font-mono uppercase text-[12px]">{data?.quantityDescriptions}</div>
                    <div className="w-[120px] text-right whitespace-pre-wrap font-mono pr-4 text-[12px]">{data?.netWeight}</div>
                    <div className="w-[120px] text-right whitespace-pre-wrap font-mono pr-4 text-[12px]">{data?.grossWeight}</div>
                </div>

                {/* Totals */}
                <div className="flex justify-end pt-4 pb-12 mb-8">
                    <div className="font-bold mr-8 mt-2">TOTAL</div>
                    <div className="w-[120px] text-right font-mono pr-4 font-bold border-b-2 border-black border-double uppercase h-8">{data?.totalNetWeight}</div>
                    <div className="w-[120px] text-right font-mono pr-4 font-bold border-b-2 border-black border-double uppercase h-8">{data?.totalGrossWeight}</div>
                </div>
                ...
                <div className="flex mb-8 uppercase">
                    ...
                    <div className="w-[20px] text-center">:</div>
                    <div className="flex-1 whitespace-pre-wrap font-bold leading-relaxed">{data?.totalText}</div>
                </div>
                <div className="flex mb-16 uppercase pb-8 border-b-2 border-slate-300">
                ...
                {/* Signatures */}
                <div className="flex justify-between items-end mt-4 mb-16">
                    <div className="flex flex-col gap-2 w-1/2 relative">
                        <div className="font-bold italic text-lg opacity-80 mb-2">N. An</div>
                        <div className="font-bold">SARINI CHUNDI</div>
                        <div className="font-bold italic">FINANCIAL</div>
                        <div className="h-10 mt-2 mb-2 font-black text-2xl italic tracking-tighter opacity-80">N. An</div>
                        <div className="uppercase">CHANIKA NANTAWANICH</div>
                        <div className="uppercase">INTERNATIONAL MARKETING OFFICER</div>
                    </div>
                    <div className="flex flex-col items-center justify-center relative pl-8 pb-10 w-1/2 text-center uppercase">
                        {/* Stamp placeholder */}
