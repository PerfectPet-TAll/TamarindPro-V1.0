...
const GLOBAL_SHEETS_CONFIG = {
    // ผู้ใช้งานระบบ
    'Users': ['id', 'username', 'password', 'name', 'role', 'email', 'status', 'lastLogin'],
    
    // ข้อมูลลูกค้า (Customers / Buyers / Consignees)
    'Customers': ['id', 'code', 'companyName', 'address', 'contactPerson', 'phone', 'email', 'country', 'status'],
    
    // ข้อมูลสินค้า (ProductsMaster)
    'Products': ['id', 'itemCode', 'brand', 'productDescription', 'nw', 'nwKg', 'packedW', 'packedWKg', 'gwKgCrt', 'standardPriceUsd', 'status'],
    
    // ข้อมูล Proforma Invoice (PI Master)
    'ProformaInvoices': ['id', 'piNo', 'refNo', 'originalDate', 'rev', 'seller', 'buyer', 'consignee', 'notifyParty', 'incoterm', 'countryOfOrigin', 'shipmentMonth', 'portOfLoading', 'portOfDischarge', 'destination', 'fobBangkok', 'freight', 'insurance', 'grandTotal', 'totalAmountLabel', 'quantityDescriptions', 'packaging', 'paymentTerm', 'deposit', 'shippingDocuments', 'validity', 'bankCharges', 'bankName', 'branch', 'address', 'accountName', 'accountNo', 'swiftCode', 'shippingMarks', 'totalNw', 'totalGw', 'totalQty', 'status', 'createdAt', 'updatedAt'],
    
    // รายการสินค้าใน Proforma Invoice (PI Items)
    'ProformaInvoiceItems': ['id', 'piId', 'shipment', 'conNo', 'productDescription', 'itemCode', 'brand', 'nw', 'nwKg', 'packedW', 'packedWKg', 'gwKgCrt', 'qtyCarton', 'totalNwKg', 'totalGwKg', 'priceUsd', 'perCtn', 'fumigation', 'amountUsd'],
    
    // ข้อมูล Quotations
    'Quotations': ['id', 'qtNo', 'date', 'customer', 'bookId', 'status', 'totalAmount', 'createdAt'],
    'SaleOrders': ['id', 'orderNo', 'date', 'customer', 'status', 'totalAmount', 'createdAt'],
    'FactoryPOs': ['id', 'poNo', 'supplier', 'date', 'status', 'totalAmount', 'createdAt'],
    'BookingRequests': ['id', 'bookingNo', 'date', 'vessel', 'voyage', 'destination', 'status', 'createdAt'],
    'BookingConfirmations': ['id', 'confirmationNo', 'bookingId', 'date', 'status', 'createdAt'],
    'ShippingInstructions': ['id', 'siNo', 'piId', 'date', 'status', 'createdAt'],
    'CommercialInvoices': ['id', 'ciNo', 'piId', 'date', 'status', 'createdAt'],
    
    // ข้อมูล Packing List (สำหรับ Export ในอนาคต)
    'PackingLists': ['id', 'piId', 'containerNo', 'sealNo', 'vessel', 'voyage', 'etd', 'eta', 'status'],
    
    // การตั้งค่าระบบ (System Config / Default Values)
...
function setupDatabase() {
  ...
  if (userSheet && userSheet.getLastRow() === 1) {
    userSheet.appendRow(['1', 'admin', '123456', 'System Admin', 'Admin', 'admin@tamarindpro.com', 'Active', '']);
  }
...
function handleLogin(ss, credentials, headersObj) {
  ...
  const user = users.find(u => 
    String(u.username) === String(credentials.username) && 
  ...
  if (user) {
    let perms = {
      ...
      canVerify: user.role === 'Admin' || user.role === 'Manager'
    };
    ...
    return createResponse("error", "Invalid Username or Password", null, headersObj);
  }
}
